...
if( $session->checkSession() ){
    ...
    #Obtener Productos
        ...
        $resultadopro = $conexion->ejecutarConsulta("
                SELECT * 
                FROM productos
                WHERE estado='ACTIVO'
            ");

            $vectorProducto = array();
            $conVecPro = 0;

            foreach($resultadopro as $fila){
                $vectorProducto[$conVecPro]['idproducto'] = $fila['idproducto'];
                $vectorProducto[$conVecPro]['nombre'] = $fila['nombre'];
                $conVecPro++;
            }
