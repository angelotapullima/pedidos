                ...
                case 1:
                    ...
                    if( opciones.length > 0 ){
                         ...
                         for(var f = 0; f < opciones.length; f++ ){
                            opcSelect += '<option ';
                            opcSelect += ' value="'+opciones[f]['nombre']+'" >';
                            opcSelect += opciones[f]['documento'];
                            opcSelect += '</option>';
                         }
...
function graficarDetallePedido(cabecera, detalle){
    ...
    pedido_detalle += '<label>N° Documento</label>';
    ...
    pedido_detalle += '<div class="form-group">';
    pedido_detalle += '<label>N° Comprobante</label>';
...
$(document).ready(function (){
    ...
    $("#formClientes").submit(function (){
        ...
        var documento = $("#clientes option:selected").text();
        var nombre = $("#clientes option:selected").val();
    ...
    $("#tablaPedidos").on("click",".imprimir_pedido", function(event){
        var tableInt = $("#tablaPedidos").DataTable();
        var datos = tableInt.row( $(this).closest('tr') ).data();

        $("#idpedcab_imprimir").val(datos['idpedcab']);

        $("#myModalImprimirPedido").modal("show");
    });

    $("#formImprimirPedido").submit(function (){
        $("#myModalImprimirPedido").modal("hide");

        var idpedcab = $("#idpedcab_imprimir").val();

        $.ajax({
            asyn: false,
            type: "POST",
            url: "util/pedidos/imprimirPedido.php",
            data: {
                idpedcab:idpedcab
            },
            dataType: 'json',
